...
def _focus_from_prompt(prompt: str) -> str | None:
    """Recover the stable scene ID carried through the persisted run prompt."""
    match = re.search(r"\[FOCUS:([^\]]+)\]", prompt, flags=re.IGNORECASE)
...
class GeminiAdkAgentProvider:
    """Managed Gemini ADK provider with ordered runtime and OpenRouter fallback."""
    ...
    async def start_run(self, context: dict[str, Any]) -> AsyncIterator[AgentEvent]:
        if not self.resources:
            if self.fallback:
                async for event in self.fallback.start_run(context):
                    yield event
                return
            raise RuntimeError("no Agent Runtime resource configured")
        ...
        message = (
            ...
            "Return a concise proposal with affected node ids, timing deltas, and "
            "the human approval required. Never mutate data.\nGRAPH_CONTEXT:\n"
        ...
        try:
            ...
            for resource in self.resources:
                try:
                    ...
                    provenance = Provenance(
                        ...
                        adapter="google-adk-agent-runtime",
                        runtime_mode="live",
                        model_name=resource.rsplit("/", 1)[-1],
                    )
                    yield AgentEvent(
                        run_id=UUID(str(context["run_id"])),
                        kind=AgentEventKind.progress,
                        message="phase:narrative",
                        payload={
                            "phase": "narrative",
                    ...
                    async for raw in agent.async_stream_query(
                        ...
                        calls = [part.get("function_call") for part in parts if isinstance(part, dict) and part.get("function_call")]
                        results = [part.get("function_response") for part in parts if isinstance(part, dict) and part.get("function_response")]
                        if calls:
                            yield AgentEvent(
                                run_id=UUID(str(context["run_id"])),
                                kind=AgentEventKind.tool_call,
                                message="phase:graph",
                                payload={"phase": "graph", "tool_calls": calls, "notes": "Managed ADK tool call."},
                                provenance=provenance,
                                occurred_at=utcnow(),
                            )
                        if results:
                            yield AgentEvent(
                                ...
                                message="phase:analytics",
                                payload={"phase": "analytics", "tool_results": results, "notes": "Managed ADK read-only tool result."},
                                provenance=provenance,
                                occurred_at=utcnow(),
                            )
                        if text_parts:
                            yield AgentEvent(
                                ...
                                message="phase:revision",
                                payload={"phase": "revision", "notes": " ".join(text_parts)},
                                provenance=provenance,
                                occurred_at=utcnow(),
                            )
                    yield AgentEvent(
                        ...
                        message="phase:critic",
                        payload={"phase": "critic", "assessment": "Gemini ADK proposal complete; human approval is required."},
                        provenance=provenance,
                        occurred_at=utcnow(),
                    )
                    return
            ...
            if self.fallback:
                async for event in self.fallback.start_run(context):
                    yield event
                return
            raise RuntimeError(f"all Agent Runtime resources failed: {last_error}")
        except Exception:
            if self.fallback:
                async for event in self.fallback.start_run(context):
                    yield event
...
def provider_for(mode: str, **credentials: Any) -> AgentProvider:
    ...
    if normalized == "live":
        ...
        if provider_key and provider_url and provider_model:
            fallback = OpenRouterAdkAgentProvider(api_key=str(provider_key), url=str(provider_url), model=str(provider_model))
        return GeminiAdkAgentProvider(
            api_key=credentials.get("api_key"),
            project=credentials.get("project"),
            location=credentials.get("location"),
            resources=tuple(credentials.get("runtime_resources") or ()),
            fallback=fallback,
        )
