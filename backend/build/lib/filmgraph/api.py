...
def _memory_scope(value: str | None) -> str:
    """Accept a bounded opaque key; never use screenplay text as memory ID."""
    ...
    if not candidate or len(candidate) > 128 or not all(char.isalnum() or char in {"-", "_", "."} for char in candidate):
        raise HTTPException(status_code=422, detail={"error_code": "memory.invalid_scope", "message": "memory_user_id must be a short opaque identifier"})
    return candidate
...
def _resolve_agent_node_reference(reference: str) -> GraphNode:
    """Resolve the UI's readable scene slug without broadening write routes."""
    wanted = reference.strip()
    if not wanted or len(wanted) > 160:
        raise HTTPException(status_code=400, detail={"error_code": "agent_tool.invalid_node", "message": "node reference is required"})
    nodes = _service().list_graph_nodes()
    lowered = wanted.lower()
    scene_match = lowered.removeprefix("scene-").removeprefix("sc ").strip()
    for node in nodes:
        if str(node.id) == wanted or str(node.id).lower() == lowered:
            return node
        if node.kind == GraphNodeKind.scene and str(node.scene_number or "").lower() == scene_match:
            return node
    raise HTTPException(status_code=404, detail={"error_code": "agent_tool.node_not_found", "message": "graph node not found"})


def _agent_node_payload(node: GraphNode) -> Dict[str, Any]:
    """Keep Agent Runtime evidence compact and read-only."""
    return {
        "id": str(node.id),
        "kind": node.kind.value,
        "scene_number": node.scene_number,
        "beat_number": node.beat_number,
        "heading": node.heading or node.label,
...
# These endpoints are the Agent Runtime's intentionally small tool surface.
# They are read-only projections over the same MCP-first/ClickHouse graph used
# by the workspace; agent tools never receive a repository write capability.
@router.get("/agent-tools/nodes/{node_reference}/impact")
def agent_tool_node_impact(node_reference: str):
    node = _resolve_agent_node_reference(node_reference)
    service = _service()
...
@router.get("/agent-tools/nodes/{node_reference}/lineage")
def agent_tool_lineage(node_reference: str):
    node = _resolve_agent_node_reference(node_reference)
    service = _service()
    lineage = service.get_lineage(node.id)
    return {
        "focus_node": _agent_node_payload(node),
        "lineage": [_agent_node_payload(item) for item in lineage[:48]],
        "query_provenance": {
            "source": service.last_source,
            "fallback": service.last_source == "direct_clickhouse",
            "error": service.last_error,
...
@router.get("/agent-tools/nodes/{node_reference}/timing")
def agent_tool_timing(node_reference: str):
    node = _resolve_agent_node_reference(node_reference)
    nodes = _service().list_graph_nodes()
    scenes = [item for item in nodes if item.kind == GraphNodeKind.scene]
    film_end = max((item.end_seconds or 0 for item in scenes), default=0)
    return {
        "focus_node": _agent_node_payload(node),
        "film_duration_seconds": film_end,
        "scene_count": len(scenes),
        "timing_source": "directly stored script timing",
    }


@router.get("/agent-tools/script/search")
def agent_tool_search_script(query: str = Query(min_length=2, max_length=120)):
    wanted = query.strip().lower()
    matches: List[GraphNode] = []
    for node in _service().list_graph_nodes():
        haystack = " ".join(filter(None, [node.label, node.heading, node.script_text, node.narration_text])).lower()
        if wanted in haystack:
            matches.append(node)
        if len(matches) >= 12:
            break
    return {"query": query, "matches": [_agent_node_payload(item) for item in matches], "match_count": len(matches)}


@router.get("/agent-tools/revision")
def agent_tool_revision():
    nodes = _service().list_graph_nodes()
    script = next((node for node in nodes if node.kind == GraphNodeKind.script), None)
    return {
        "film_id": script.film_id if script else "unknown",
        "revision_id": script.revision_id if script else "unknown",
        "source_heading": script.heading if script else "Script",
...
def create_agent_run(request: AgentRunCreateRequest) -> AgentRun:
    ...
    memory_user_id = _memory_scope(request.memory_user_id)
    run = _service().create_agent_run(
        request.workflow_id,
        prompt,
        request.stage_name,
        model_name,
    ...
    _service().append_agent_event(
        ...
        f"{request.runtime_mode}: agent run started",
        {
            ...
            "memory": {"scope": "user_id", "user_id": memory_user_id, "status": "managed_memory_bank_configured"},
        },
        request.runtime_mode,
    )
...
async def stream_agent_run(run_id: UUID) -> StreamingResponse:
    ...
    provider = provider_for(
        ...
        runtime_resources=settings.agent_runtime_resources,
    )
        ...
        if len(existing) <= 1:
            try:
                context = {
                    ...
                    "memory_user_id": run.memory_user_id,
                }
